...
const defaultProductionClientPath = '/home/investu1/public_html';
const clientPath = process.env.CLIENT_DIST_PATH || (fs.existsSync(localClientDist)
  ? localClientDist
  : defaultProductionClientPath);

if (fs.existsSync(clientPath)) {
  console.log(`Serving React app from: ${clientPath}`);
  app.use(express.static(clientPath));
  // SPA fallback: only serve index.html for routes without file extensions
  ...
  console.warn(`Client build folder not found. Expected either ${localClientDist} or ${defaultProductionClientPath}.`);
  if (process.env.CLIENT_DIST_PATH) {
    console.warn(`CLIENT_DIST_PATH is set to ${process.env.CLIENT_DIST_PATH}`);
  }
  app.get('/', (req, res) => {
    res.send('InvestUp API is running successfully.');
  });
}
