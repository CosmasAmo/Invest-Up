...
function Users() {
    const { users, fetchAllUsers, createUser, deleteUser, updateUser, fetchReferralCodes, generateMissingReferralCodes } = useAdminStore();
    ...
    const isUserAdmin = isAdmin || userData?.isAdmin === true || localStorage.getItem('is_admin') === 'true';

    // Debug admin status on component mount
    useEffect(() => {
        console.log('=== ADMIN STATUS DEBUG ===');
        console.log('Store isAdmin:', isAdmin);
        console.log('userData?.isAdmin:', userData?.isAdmin);
        console.log('localStorage is_admin:', localStorage.getItem('is_admin'));
        console.log('Final isUserAdmin:', isUserAdmin);
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                console.log('Admin status check:', { 
                    ...
                    localStorageIsAdmin: localStorage.getItem('is_admin'),
                    finalIsAdmin: isUserAdmin 
                });
                
                // First ensure admin auth is verified
                setIsLoading(true);
                ...
                while (retryCount < maxRetries) {
                    try {
                        ...
                        if (retryCount >= maxRetries) {
                            toast.error('Failed to load users after multiple attempts. Please try refreshing the page.');
                            break;
                        }
        ...
        // Only show warning if user is definitely not an admin and we've had time to load the data
        setTimeout(() => {
            const finalAdminCheck = isAdmin || userData?.isAdmin === true || localStorage.getItem('is_admin') === 'true';
            if (!finalAdminCheck) {
        ...
        const refreshInterval = setInterval(() => {
            ...
            fetchAllUsers().catch(err => console.error('Error in refresh interval:', err));
        }, 60000); // Refresh every minute
        
        return () => clearInterval(refreshInterval);
    }, [fetchAllUsers, fetchReferralCodes, isAdmin, userData?.isAdmin, isUserAdmin]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Remove financial fields that should be calculated from actual records
            delete submitData.totalInvestments;
            delete submitData.totalProfits;
            delete submitData.totalWithdrawals;
            
            // Only allow setting balance for admin users
            if (!submitData.isAdmin) {
                delete submitData.balance;
            }
            
            // If editing an admin user, preserve their balance
            if (editingUser && submitData.isAdmin && editingUser.balance) {
                submitData.balance = editingUser.balance;
            }
            ...
            if (hasNewProfileImage) {
                ...
                Object.keys(submitData).forEach(key => {
                    formDataWithImage.append(key, submitData[key]);
                });
            ...
            <div className="flex justify-between items-start">
                <div className="flex items-center space-x-3 max-w-[65%]">
                    <div className={`min-w-[48px] h-12 w-12 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg overflow-hidden ${
                        ...
                        {/* Display user initials as fallback instead of profile image */}
                        {user.name ? (
                            <span>{user.name.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2)}</span>
                        ) : (
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                            ...
                            {formData.role !== 'admin' && (
                                ...
                                <div>
                                    ...
                                    <div className="w-full px-4 py-2 bg-slate-800 text-white rounded-lg border border-slate-600 flex items-center">
                                        <span>${formData.balance}</span>
                                        <span className="ml-2 text-xs text-slate-400">(Read-only, calculated from transactions)</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="w-full px-4 py-2 bg-slate-800 text-white rounded-lg border border-slate-600 flex items-center">
                                        <span>${formData.totalInvestments}</span>
                                        <span className="ml-2 text-xs text-slate-400">(Read-only, calculated from investments)</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="w-full px-4 py-2 bg-slate-800 text-white rounded-lg border border-slate-600 flex items-center">
                                        <span>${formData.totalProfits}</span>
                                        <span className="ml-2 text-xs text-slate-400">(Read-only, calculated from investments)</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="w-full px-4 py-2 bg-slate-800 text-white rounded-lg border border-slate-600 flex items-center">
                                        <span>${formData.totalWithdrawals}</span>
                                        <span className="ml-2 text-xs text-slate-400">(Read-only, calculated from withdrawals)</span>
                                    </div>
                                </div>
                                </>
                            )}
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                                <div className="flex items-center gap-4">
                                    <div className="w-full sm:w-52 sm:h-52 rounded-lg bg-slate-700 flex items-center justify-center overflow-hidden border border-slate-600 mx-auto mb-4">
