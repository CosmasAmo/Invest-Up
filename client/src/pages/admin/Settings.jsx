...
function Settings() {
    const [settings, setSettings] = useState({
        ...
        withdrawalFee: 2,
        referralsRequired: 2,
        depositAddresses: {} // Populated from server .env at runtime – never hardcoded here
    });
    ...
    const fetchSettings = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                // depositAddresses come from server .env – never inject fallbacks here
                ...
                // Note: we intentionally do NOT cache depositAddresses in localStorage
            }
        } catch (error) {
            console.error('Failed to fetch settings:', error);
            // If API fails, show a toast – do NOT fall back to hardcoded addresses
            toast.error('Could not load settings from server. Please check your connection.');
            try {
                const savedSettings = localStorage.getItem('adminSettings');
                if (savedSettings) {
                    const parsedSettings = JSON.parse(savedSettings);
                    // Never inject hardcoded addresses from localStorage
                            ...
                            <div className="pt-4 border-t border-slate-700 mt-8">
                                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                                    ...
                                    <button
                                        ...
                                        className={`w-full sm:w-auto px-6 py-3 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 ${saveSuccess
