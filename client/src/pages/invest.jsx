...
function Invest() {
  const { submitInvestment, isLoading: isSubmitting, userData, stats, fetchUserDashboard } = useStore();
    ...
    try {
      ...
      if (result.success) {
        toast.update(toastId, {
          render: "Investment submitted successfully!",
          type: "success",
          isLoading: false,
          autoClose: 3000
        });
        ...
        toast.update(toastId, {
          render: result.message || "Failed to submit investment. Please try again.",
          type: "error",
          isLoading: false,
          autoClose: 3000
        });
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
                <div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                    ...
                    <div>
                      <button
                        type="submit"
                        disabled={isSubmitting || !amount || parseFloat(amount) < getMinimumInvestmentAmount() || parseFloat(amount) > userBalance}
                        className={`w-full py-2.5 sm:py-3 px-3 sm:px-4 rounded-lg font-medium text-sm sm:text-base text-white ${
                          isSubmitting || !amount || parseFloat(amount) < getMinimumInvestmentAmount() || parseFloat(amount) > userBalance
                ...
                <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-xl p-4 sm:p-6 border border-slate-700/30">
                  ...
                  <div className="mt-5 p-3 sm:p-4 bg-slate-800/50 rounded-lg border border-slate-700/50">
                    ...
                    <p className="text-xl sm:text-2xl font-bold text-white">${userBalance.toFixed(2)}</p>
