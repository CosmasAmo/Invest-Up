...
import { FaFilter, FaSearch, FaDownload, FaCalendarAlt, FaChevronLeft, FaChevronRight, FaExclamationCircle, FaSpinner, FaExternalLinkAlt, FaFilePdf, FaFileExcel, FaFileCsv } from 'react-icons/fa'
...
function Transactions() {
  const { userData, fetchDashboardData, stats, deposits, withdrawals, investments } = useStore()
  ...
  // Create transactions from store data if API fails
  const createTransactionsFromStoreData = useCallback(() => {
    console.log('Creating transactions from store data');
    console.log('Deposits:', deposits.length);
    console.log('Withdrawals:', withdrawals.length);
    console.log('Investments:', investments.length);
    
    // Combine all transactions from store data
    ...
    console.log(`Created ${combinedTransactions.length} transactions from store data`);
    return combinedTransactions;
  }, [deposits, withdrawals, investments]);
  
  // Store stable references to functions to avoid dependency changes
  const stableFetchDashboardData = useRef(fetchDashboardData).current;
  const stableCreateTransactions = useRef(createTransactionsFromStoreData).current;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        console.log('Starting to fetch dashboard data and transactions');
        // First fetch dashboard data to ensure latest stats
        await stableFetchDashboardData();
        
        // Use the store's fetchTransactions function directly
        const transactions = await useStore.getState().fetchTransactions();
        ...
        if (transactions && transactions.length > 0) {
          ...
          console.error('No transactions returned from store function');
          
          // Fallback: try to get transactions from userData
          if (userData?.recentTransactions && userData.recentTransactions.length > 0) {
            console.log('Using fallback recentTransactions from userData:', userData.recentTransactions.length);
            setTransactions(userData.recentTransactions);
          } else {
            // If still no transactions, create from store data
            console.log('No recentTransactions, creating from store data');
            const storeTransactions = stableCreateTransactions();
            if (storeTransactions.length > 0) {
              console.log('Using transactions created from store data:', storeTransactions.length);
              setTransactions(storeTransactions);
            } else {
              console.error('No transactions available from any source');
              setError('No transaction history available.');
            }
          }
        }
      } catch (error) {
        ...
        // 1. First try recent transactions from userData
        if (userData?.recentTransactions && userData.recentTransactions.length > 0) {
          console.log('Using fallback recentTransactions from userData after error:', userData.recentTransactions.length);
          setTransactions(userData.recentTransactions);
        } 
        // 2. Otherwise create transactions from store data
        else {
          console.log('No recentTransactions after error, creating from store data');
          const storeTransactions = stableCreateTransactions();
          if (storeTransactions.length > 0) {
            console.log('Using transactions created from store data after error:', storeTransactions.length);
            setTransactions(storeTransactions);
          } else {
            console.error('No transactions available from any source after error');
            setError('No transaction history available.');
          }
        }
      } finally {
    ...
    // Add a debounce timeout to avoid excessive API calls
  ...
  const exportToPDF = () => {
    try {
      ...
      sortedTransactions.forEach(transaction => {
        ...
        const type = transaction.type ? transaction.type.charAt(0).toUpperCase() + transaction.type.slice(1) : 'Unknown';
        const amount = `$${parseFloat(transaction.amount).toFixed(2)}`;
        const status = transaction.type === 'investment' && transaction.status === '' 
          ? 'Stopped' 
          : transaction.status 
  ...
  const exportToExcel = () => {
    try {
      ...
      const excelData = sortedTransactions.map(transaction => ({
        ...
        Type: transaction.type ? transaction.type.charAt(0).toUpperCase() + transaction.type.slice(1) : 'Unknown',
        Amount: `$${parseFloat(transaction.amount).toFixed(2)}`,
        Status: transaction.type === 'investment' && transaction.status === '' 
          ? 'Stopped' 
          : transaction.status 
            ? transaction.status.charAt(0).toUpperCase() + transaction.status.slice(1) 
            : 'Unknown',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16 sm:pb-20">
        <motion.div
