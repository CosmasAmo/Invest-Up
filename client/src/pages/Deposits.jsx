...
import { FaPlus, FaHistory, FaExclamationCircle, FaSpinner, FaFilter, FaSearch, FaCalendarAlt, FaTimes, FaArrowLeft, FaEyeSlash, FaEye } from 'react-icons/fa';
import axios from 'axios';
import { toast } from 'react-toastify';

function Deposits() {
  const { deposits = [], totalDepositsAmount = 0, fetchDeposits, isLoading, error } = useStore();
  ...
  const filteredDeposits = deposits.filter(deposit => {
    ...
    // Filter hidden deposits unless showHidden is true
    if (deposit.hidden && !showHidden) return false;
  ...
  // Count hidden deposits
  const hiddenCount = deposits.filter(d => d.hidden).length;
  ...
  const handleToggleHidden = async (deposit) => {
    setIsSubmitting(true);
    
    try {
      const response = await axios.patch(`/api/transactions/deposit/${deposit.id}/toggle-hidden`);
      ...
      console.error('Error toggling deposit visibility:', error);
    } finally {
  ...
  // Format the total amount deposited from the store
  const formattedTotalDeposited = parseFloat(totalDepositsAmount).toFixed(2);
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          >
            ...
            <p className="text-2xl font-bold text-white">{deposits.filter(d => d.status !== 'deleted').length}</p>
          </motion.div>
          ...
          >
            ...
            <p className="text-2xl font-bold text-white">${formattedTotalDeposited}</p>
          </motion.div>
        </div>
        ...
        >
          ...
          {/* Toggle button for hidden deposits */}
          <div className="mt-4">
            <button
              onClick={() => setShowHidden(!showHidden)}
              className={`flex items-center px-4 py-2 rounded-lg border ${showHidden ? 'bg-amber-600/20 text-amber-400 border-amber-500/30' : 'bg-slate-700 text-slate-300 border-slate-600'} transition-colors`}
            >
              {showHidden ? (
                <>
                  <FaEye className="w-4 h-4 mr-2" />
                  <span>Showing Hidden Deposits ({hiddenCount})</span>
                </>
              ) : (
                <>
                  <FaEyeSlash className="w-4 h-4 mr-2" />
                  <span>Show Hidden Deposits ({hiddenCount})</span>
                </>
              )}
            </button>
          </div>
        ...
        {isLoading ? (
          ...
          >
            {sortedDeposits.map((deposit) => (
              <DepositCard key={deposit.id} deposit={deposit} onEdit={handleEditClick} onDelete={handleDeleteClick} onToggleHidden={handleToggleHidden} />
            ))}
          </motion.div>
        ) : (
          <motion.div 
          ...
          >
            {searchTerm || filter !== 'all' ? (
              ...
              <>
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No Deposits to display</h3>
                <p className="text-slate-400 mb-6">You haven&apos;t made any deposits yet or you have hidden the deposits.</p>
                <Link 
                  to="/deposit"
