...
import { FaUser, FaEnvelope, FaShieldAlt, FaKey, FaCheck, FaExclamationTriangle, FaCalendarAlt, FaEdit, FaSave, FaTimes, FaUserCircle } from 'react-icons/fa'
    ...
    try {
      ...
      if (success) {
        ...
        try {
          ...
          if (store && typeof store.refreshUserProfile === 'function') {
            const refreshSuccess = await store.refreshUserProfile();
            if (refreshSuccess) {
              console.log('Profile refreshed successfully');
            }
          } else {
            console.warn('refreshUserProfile function not available');
          }
        } catch (refreshError) {
        ...
        const store = useStore.getState();
        const errorMessage = store.error || 'Failed to update profile. Please try again.';
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/4 bg-slate-900/80 p-4 sm:p-6 backdrop-blur-sm">
              <div className="flex flex-col items-center mb-6 sm:mb-8">
                <div className="w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center bg-slate-700 rounded-full mb-3">
                  <FaUserCircle className="text-gray-400 w-12 h-12 sm:w-16 sm:h-16" />
                </div>
