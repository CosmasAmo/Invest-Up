...
function Deposit() {
  ...
  const getMethodDescription = (methodKey) => {
    switch (methodKey) {
      case 'BINANCE': return 'Deposit USDT to this Binance ID';
      case 'TRC20': return 'Send USDT via Tron network';
      case 'BEP20': return 'Send USDT via BNB Smart Chain';
      case 'ERC20': return 'Send USDT via Ethereum network';
      case 'OPTIMISM': return 'Send USDT via Optimism network';
      default: return `Send USDT via ${methodKey} network`;
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      ...
      try {
        // Always fetch the latest settings from the server
        const response = await axios.get('/api/settings/public');
        
        if (response.data.success) {
          console.log('Settings received from API:', response.data.settings);
          ...
          setSettings({
            minDeposit: fetchedSettings.minDeposit || 3,
          ...
          console.log('Setting deposit methods:', methods);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Show a loading toast that will be updated on success/error
      const toastId = toast.loading("Processing your deposit...");
      
      const success = await submitDeposit(formData);
      if (success) {
        toast.update(toastId, {
          render: "Deposit submitted successfully. Awaiting admin approval.",
          type: "success",
          isLoading: false,
          autoClose: 3000
        });
        // Redirect is now handled in the store function
      }
    } catch (error) {
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          {Object.keys(depositMethods).length === 0 ? (
                            ...
                            Object.entries(depositMethods).map(([key, method]) => (
                              <div
                                key={key}
                                onClick={() => amount && parseFloat(amount) >= settings.minDeposit ? handleMethodSelect(key) : null}
                                className={`p-4 rounded-xl border transition-all duration-200 hover:shadow-md ${
                                  amount && parseFloat(amount) >= settings.minDeposit
                                    ? 'border-slate-600 hover:border-blue-500 bg-slate-900/70 hover:bg-slate-800/70 cursor-pointer'
