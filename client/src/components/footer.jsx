...
function Footer() {
  ...
  const contactInfo = [
    ...
    { icon: <FaEnvelope />, text: 'investup.support@investuptrading.com' },
            ...
            <ul className="space-y-3">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  <Link
                    to={link.path}
                    className="text-gray-400 hover:text-blue-400 transition-colors text-sm lg:text-base"
                  >
                    {link.label}
                  </Link>
                </li>
            ...
            <ul className="space-y-4">
              {contactInfo.map((info, index) => (
                <li key={index} className="flex items-start gap-3 text-gray-400">
                  <span className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5">{info.icon}</span>
                  <span className="text-sm lg:text-base leading-relaxed" style={{ wordBreak: 'break-word' }}>{info.text}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 pt-8 border-t border-slate-800">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-6 text-gray-400">
              <Link to="/terms" className="hover:text-blue-400 transition-colors text-sm lg:text-base">
                Terms of Service
              </Link>
            </div>
          </div>
