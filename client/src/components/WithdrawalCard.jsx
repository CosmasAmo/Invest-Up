...
import { FaWallet, FaCalendarAlt, FaEdit, FaTrash, FaExchangeAlt, FaEyeSlash, FaEye } from 'react-icons/fa';

function WithdrawalCard({ withdrawal, onEdit, onDelete, onToggleHidden }) {
  ...
  // Don't render hidden withdrawals unless explicitly shown
    ...
    >
      <div className="flex flex-wrap justify-between items-start mb-4 gap-2">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {/* Hide/Unhide button for approved withdrawals */}
          {withdrawal.status === 'approved' && (
            <button
              onClick={() => onToggleHidden(withdrawal)}
              className={`p-1.5 rounded-md ${withdrawal.hidden ? 'bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 border border-blue-500/30' : 'bg-amber-500/20 text-amber-400 hover:bg-amber-500/30 border border-amber-500/30'} transition-colors`}
              title={withdrawal.hidden ? "Unhide Withdrawal" : "Hide Withdrawal"}
              aria-label={withdrawal.hidden ? "Unhide Withdrawal" : "Hide Withdrawal"}
            >
              {withdrawal.hidden ? <FaEye className="w-3.5 h-3.5" /> : <FaEyeSlash className="w-3.5 h-3.5" />}
            </button>
          )}
        </div>
      </div>
