...
const useAdminStore = create((set) => ({
    ...
    fetchAdminStats: async (timeRange = 'week') => {
        ...
        try {
            const { data } = await axiosInstance.get(`/api/admin/stats?timeRange=${timeRange}`);
            if (data.success) {
    ...
    fetchRecentTransactions: async () => {
        set({ isLoading: true, error: null });
        try {
            const { data } = await axiosInstance.get('/api/admin/transactions/recent');
            if (data.success) {
    ...
    fetchAllUsers: async () => {
        ...
        try {
            ...
            const { data } = await axiosInstance.get(`/api/admin/users?_=${timestamp}`);
            if (data.success) {
    ...
    fetchReferralCodes: async () => {
        set({ isLoading: true, error: null });
        try {
            const { data } = await axiosInstance.get('/api/admin/users/referral-codes');
    ...
    updateUserStatus: async (userId, isVerified) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/update-user-status', {
                userId,
                isVerified
            });
            if (data.success) {
    ...
    fetchPendingDeposits: async () => {
        try {
            const { data } = await axiosInstance.get('/api/admin/pending-deposits');
            if (data.success) {
    ...
    handleDeposit: async (depositId, status) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/handle-deposit', {
                depositId,
                status
            });
            if (data.success) {
    ...
    fetchApprovedDeposits: async () => {
        try {
            const { data } = await axiosInstance.get('/api/admin/approved-deposits');
            if (data.success) {
    ...
    fetchPendingInvestments: async () => {
        try {
            const { data } = await axiosInstance.get('/api/admin/pending-investments');
            if (data.success) {
    ...
    handleInvestment: async (investmentId, status) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/handle-investment', {
                investmentId,
                status
            });
            if (data.success) {
    ...
    fetchApprovedInvestments: async () => {
        try {
            const { data } = await axiosInstance.get('/api/admin/approved-investments');
            if (data.success) {
    ...
    fetchPendingWithdrawals: async () => {
        try {
            const { data } = await axiosInstance.get('/api/admin/pending-withdrawals');
            if (data.success) {
    ...
    handleWithdrawal: async (withdrawalId, status) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/handle-withdrawal', {
                withdrawalId,
                status
            });
            if (data.success) {
    ...
    markMessageAsRead: async (messageId) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/mark-message-read', { messageId });
            if (data.success) {
    ...
    createUser: async (userData, isFormData = false) => {
        ...
        try {
            ...
            const { data } = await axiosInstance.post('/api/admin/users', userData, config);
            if (data.success) {
                return data.user;
            } else {
                if (data.message && (
    ...
    updateUser: async (userId, userData, isFormData = false) => {
        ...
        try {
            ...
            const { data } = await axiosInstance.put(`/api/admin/users/${userId}`, userData, config);
            if (data.success) {
    ...
    fetchMessages: async () => {
        try {
            const { data } = await axiosInstance.get('/api/admin/messages');
            if (data.success) {
    ...
    replyToMessage: async (messageId, reply) => {
        ...
        try {
            const { data } = await axiosInstance.post('/api/admin/reply-message', {
                messageId,
                reply
            });
    ...
    testEmail: async (testEmailAddress) => {
        set({ isLoading: true, error: null });
        try {
            const { data } = await axiosInstance.post('/api/admin/test-email', {
    ...
    markAsRead: async (messageId) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/mark-as-read', {
                messageId
            });
            if (data.success) {
                set(state => ({
    ...
    broadcastMessage: async (subject, message) => {
        try {
            const { data } = await axiosInstance.post('/api/admin/broadcast', { subject, message });
            return data;
        } catch (error) {
    ...
    generateMissingReferralCodes: async () => {
        set({ isLoading: true, error: null });
        try {
            const { data } = await axiosInstance.post('/api/admin/generate-missing-referral-codes');
            if (data.success) {
                set({ isLoading: false });
                return data;
            } else {
                throw new Error(data.message || 'Failed to generate referral codes');
            }
        } catch (error) {
